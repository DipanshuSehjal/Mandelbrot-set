...
def mandelbrot_static(xmin, xmax, ymin, ymax, N=100):
    ...
    while i < mp.cpu_count():
        ...
        if i > count:
            p = mp.Process(target=call_mandelbrot_serial, args=(x, x + incx, ymin, 0, N//count, results, i))
            p.start()
            processes.append(p)
            x += incx
        else:
            p = mp.Process(target=call_mandelbrot_serial, args=(x, x + incx, 0, ymax, N//count, results, i))
            p.start()
            processes.append(p)
            x += incx
